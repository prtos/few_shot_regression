...
python ../shared/generate_expt_config.py -d toy -a metakrr_mk metakrr_sk -o ${config_file}
bash  ../shared/build_personal_image.sh -n ${image_name} -i content.txt
dispatcher -n ${image_name} -e ${image_name} -t ${instance_type} -c ${nb_instances} -x 72000  -o ${out} -p ${config_file}
