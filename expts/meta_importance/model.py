...
def rf_regression_std(regressor, X):
    ...
    query_idx = np.argmax(np.std(temp, axis=1))
    return query_idx, X[query_idx]
...
class BaseModel(BaseEstimator):
    ...
    def to_fp(self, x):
        ...
        res = fp_feat(x, dtype=np.float32, ignore_errors=False)
        return res
...
# <class 'numpy.ndarray' > <class 'method' > <class 'ivbase.transformers.scaler.NoneScaler' > <class 'sklearn.ensemble.forest.RandomForestRegressor' >
# <class 'numpy.ndarray' > <class 'method' > <class 'ivbase.transformers.scaler.NoneScaler' > <class 'sklearn.ensemble.forest.RandomForestRegressor' >
...
class ActiveModel(BaseModel):
    def __init__(self, *args, **kwargs):
        ...
        if self.model_name == 'rf_rgr':
            self.strategy = rf_regression_std
        elif self.model_name == 'gp_rgr':
            self.strategy = gp_regression_std
        else:
            raise Exception("No strategy in place for the specified model name")

    def fit(self, X, y, initial_train_size, n_queries, param_grid=None, x_scaler='none',
        ...
        active_model = ActiveLearner(
            ...
            query_strategy=self.strategy,
            X_training=x_train, y_training=(y_train.reshape(-1, 1) if len(y_train.shape) == 1 else y_train)
        )

        best_score, last_decrease = None, 0
        for i in range(n_queries):
            query_idx, query_instance = active_model.query(x_pool)
            active_model.teach(x_pool[query_idx].reshape(1, -1), y_pool[query_idx].reshape(1, -1))
            # Remove the queried instance from the unlabeled pool.
            x_pool, y_pool = np.delete(x_pool, query_idx, axis=0), np.delete(y_pool, query_idx, axis=0)
            preds = active_model.predict(x_pool)
