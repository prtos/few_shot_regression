...
class MetaRegressionDataset(MetaDataset):
    ...
    def __init__(self, tasks_filenames, x_transformer=None, y_transformer=None, task_descriptor_transformer=None,
                 max_examples_per_episode=None, max_test_examples=None,
        ...
        self.__dir__()
        self.input_params = {k: v for k, v in temp.items() if k not in ['self', 'tasks_filenames'] and not k.startswith('__')}
        ...
        for file_index, epfile in enumerate(self.tasks_filenames):
            ...
            if self.mode == self.TRAIN:
                ...
                while i < 100:
                    ...
                    if len(train_indexes) == ksize and np.std(y[train_indexes]) > 0:
                        j += 1
                        test_indexes = all_examples_indexes_list[ksize:ksize+self.max_test_examples]
                        self.episodes_samples_indexes.append((file_index, train_indexes, test_indexes))
                    i += 1
            mm += n_examples
    ...
    def train_test_split(self, test_size):
        ...
        metatrain = self.__class__(metatrain_episode_files, **self.input_params)
        metatest = self.__class__(metatest_episode_files, **self.input_params)
