...
class GPLearner(torch.nn.Module):
    ...
    def fit(self, phis, y):
        ...
        # self.L_ = torch.potrf(self.K_ + self.pre_l2 * I, upper=False)
    ...
    def log_marginal_likelihood(self):
        ...
        try:
            chod = torch.potrf(self.K_ + self.l2 * I, upper=False)
        except:
            print('Warning: an inversion error occurs')
            chod = torch.potrf(self.K_ + self.l2 * 100 * I, upper=False)
        lml -= torch.log(chod.diag()).sum()
        lml -= 0.5 * self.y_train.size(0) * np.log(2 * np.pi)
    ...
    def forward(self, phis):
        ...
        # print(phis.size(), y_mean.size(), torch.std(y_var).view(-1, 1).size())
        return torch.cat([y_mean, torch.sqrt(y_var).view(-1, 1)], dim=1)
