...
import pandas as pd
import numpy as np
from few_shot_regression.utils.datasets import loaders
import few_shot_regression.utils.feature_extraction as extraction
import few_shot_regression.models as models
...
def get_outfname_prefix(expts_dir, all_params):
    ...
    prefix = "{}/{}".format(expts_dir, readable_name.lower()+uid)
    params_fname = "{}_params.txt".format(prefix)
    with open(params_fname,  'w') as fp:
        json.dump(params_flatten, fp, indent=4, sort_keys=True)

    return prefix


def get_dataset_partitions(dataset_name, dataset_params):
    assert dataset_name in loader_dict, 'Unknown dataset'
    return loader_dict[dataset_name](**dataset_params)
